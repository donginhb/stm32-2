...
void TSI_Init(void)
{
...
#ifdef TWR_K60N512   //修改，以适应本电路板原理
  PORTC_PCR2 = PORT_PCR_MUX(0);       //Enable ALT0 for portC1
  PORTC_PCR1 = PORT_PCR_MUX(0);       //Enable ALT0 for portc2
  PORTB_PCR19 = PORT_PCR_MUX(0);      //Enable ALT0 for portB18
  PORTB_PCR18 = PORT_PCR_MUX(0);      //Enable ALT0 for portB19
  ...
  TSI0_GENCS |= ((TSI_GENCS_NSCN(10))|(TSI_GENCS_PS(3)));//定义扫描次数和电极振荡器分频设置
  TSI0_SCANC |= ((TSI_SCANC_EXTCHRG(3))|(TSI_SCANC_REFCHRG(31))|(TSI_SCANC_DELVOL(7))|(TSI_SCANC_SMOD(0))|(TSI_SCANC_AMPSC(0)));
//定义外部振荡器充电电流、参考振荡器充电电流选择、应用于模拟振荡器的三角波电压选择，扫描模数，激活模式下分频器选择
...
/********************************************************************************
 *   TSI_SelfCalibration: Simple auto calibration version  简单的自动校准版本
 * Notes:
