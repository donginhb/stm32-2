...
static unsigned char myip[4] = {192,168,1,8};

/* ip地址(或者是DNS的名字，如果有DNS服务器的话)，ip地址必须以"/"结尾 */
static char baseurl[]="http://192.168.1.8/";
...
unsigned int print_webpage(unsigned char *buf,unsigned char on_off)
{
    ...
    plen=fill_tcp_data_p(buf,plen,PSTR("</center><hr><br>****************^_^ 欢迎您使用陆航MiniStm32开发板 ^_^****************\n"));
 ...
 * 输出  ：无
 * 返回  ：-0 运行成功
 * 应用  ：1 在PC机的DOS界面输入： ping 192.168.1.8 (看能否ping通)
