...
**  联系：751956552@qq.com
**  修改日期：2012.12.28
**  说明：本文件为红外芯片IR01F的底层驱动，不允许客户进行改动，
          如改动后请不要向我们报错！
**************************飞音云电子******************************/
...
kal_uint8	Get_Learn_Data(unsigned char*learn_data_out);
...
//===============================================================================================================
//处理函数:
//in:	readtempbuf[]    110B		读回的110B的数据:原始,没处理过的,十六进制
//out: 	learn_data_out[] 112B		最后处理完成的包括:CHECKSUM,最后发学习数据是:0X30+0X20+learn_data_out[110]  =112个字节,十六进制
//===============================================================================================================
kal_uint8	Get_Learn_Data(unsigned char*learn_data_out)
	{	
		kal_uint8 	tempw;
		kal_uint8          tempct;			//增加这个变量
		char	data_check=0;			//增加这个变量
		int i=0;
	
		unsigned char	 g_remote_study_data_buf[110];	
						
		tempct = get_remote_study_data((unsigned char *)learn_data_out);	
		...
		if(!tempct)
			...
			send_remote_study_data((unsigned char *)g_remote_study_data_buf,109);	//wjs;;
...
//=====================数据处理分以下两个步骤		
//过程1=============================================================
//in:		发回的110B的数据:原理,没处理过的:readtempbuf[]
//out:cmd_data[]		处理过程中暂时存的数据
kal_uint8 get_remote_study_data(unsigned char *cmd_data)
{
	char readtempbuf[110];
	unsigned int irprocessbuf[111];
	kal_uint8 temp;
	unsigned int datalength;

	temp=readI2C(readtempbuf);		//清readtempbuf
	...
	if(!temp)
		...
		for(temp=0; temp<109+1; temp++)
			{
			irprocessbuf[temp] = (unsigned int)readtempbuf[temp];		///wjs;readtempbuf,irprocessbuf原始110Ｂ:未处理
			}		
		datalength = 109+1;
		...
		if(0!=(compdata(irprocessbuf, &datalength)))
			...
			for(temp=0; temp<109; temp++)		//wjs;datalengt
				{
				cmd_data[temp] = (unsigned char )irprocessbuf[temp+1];		//wjs;cmd_data[]:110B处理过程中的数据:109B
				}				
			}
...
////过程2=============================================
//in:过程1的输出:cmd_data[]  ,110B
//out:g_i2c_cmd_buffer
/******************************************************************************
enter:	char *cmd_data --> 数据地址
return:  	 = 0 send ok 
		!= 0 send error
******************************************************************************/
kal_uint8	g_toggle_bit=0;

kal_uint8 send_remote_study_data(unsigned char *cmd_data, U16 cmd_lnth)		//wjs;cmd_lnth=109
{
	unsigned int sendlrndat[109+1], sendlrndatsum;
	//char g_i2c_cmd_buffer[109+3] = "";
	unsigned int toggle;
	kal_uint8 temp;
	char checksum = 0x32;	///40;	//wjs;YL:START_WRITE_I2C_DADDR + SEND_STUDY_REMOTE_DATA;
				//wjs;=0x30+0x02
	...
	toggle=keytogglebit(sendlrndat,&sendlrndatsum);	//wjs;包括了各种数据处理
...
//	Send_IR_Data(g_i2c_cmd_buffer, templnth);
}
/////=========================================================后补的子函数
//void	keytogglebit(unsigned int *wavedat,unsigned int *indatsum)
unsigned	int	keytogglebit(unsigned int *wavedat,unsigned int *indatsum)		///wjs;IN:sendlrndat[] 110B  Y
{
	unsigned	int	tmp1;
	unsigned	int	tmp2;
					...
					while(1){	//ToggleBit_Judge_BIT
						...
						if(0==acmpbandc(cmpa,cmpb,cmpc)){
							...
							if(0==acmpbandc(cmpa,cmpb,cmpc)){
								...
								cmpc*=0x100;
								cmpc+=togtmp1;
								if(0==acmpbandc(cmpa,cmpb,cmpc)){	//ToggleBit_Judge_OneBITHigh1
									togtmp6=0;
									togtmp5=wavedat[cntx++];
									if(togtmp5>=0x80){
										togtmp6=togtmp5;
...
//数据处理函数
//in：采集到的数据数组变量：wavedat[]   -------(这个值现变成调用这个方法时给的参数
//out: 处理后的数据，wavedat[]，要另存--------(这个值变成不是最终处理后的值
