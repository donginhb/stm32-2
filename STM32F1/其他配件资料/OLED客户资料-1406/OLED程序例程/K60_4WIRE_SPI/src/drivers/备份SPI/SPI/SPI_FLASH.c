...
{		
  ...
  return SPI_Master_WriteRead(SPI0,TxData,SPI_PCS4,SPI_PCS_INACTIVE);
...
unsigned char TxBuffer[10];//串口发送缓冲区
unsigned char RxBuffer[1000]; //串口接收缓冲区
...
void TestFlash (void)
{
  ...
  unsigned char FlashBuffer1[0x100];
  
  unsigned char FlashBuffer2[0x100];
  ...
  if(SPI_Flash_ReadID()==FLASH_ID)//检测不到W25X16
	{	   
	  
	  TxBuffer[0]=FLASH_ID>>8;
	  TxBuffer[1]=(unsigned char)FLASH_ID&0xff;
	  //DATA_send(TxBuffer,2);
	  uart_sendN (3, TxBuffer, 2);
  ...
  i=AH_CRC_Verify(FlashBuffer1,0xFE,1 );

  for(i=0;i<0x2000;i++)
  {


  	SPI_Flash_Write_Page(FlashBuffer1,addr,0x100);//SPI在一页(0~65535)内写入少于256个字节的数据
  	
	Init_Buffer (FlashBuffer2,0x100,0);
	
	SPI_Flash_Read(FlashBuffer2,addr,0x100);//读取SPI FLASH 在指定地址开始读取指定长度的数据   
	
	CRCJG=AH_CRC_Verify(FlashBuffer2,0xFE,2);
	...
	{
	  //DATA_send (FlashBuffer2,0x100); 
          uart_sendN (3, TxBuffer, 0x100);
