/**********************************************
File Name: ALIENTEK MINISTM32 实验14 内部温度传感器实验(库)
Revision:
Corporation:
...
//使用SW调试模式,如使用JLINK JTAG下载的请在下载完后拔走JLINK
...
int main(void)
	...
	//ADC_RegularChannelConfig(ADC1, ADC_Channel_16, 5, ADC_SampleTime_239Cycles5 );//ADC1,ADC通道16,规则采样顺序值为5,采样时间为239.5周期		
