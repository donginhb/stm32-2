/**********************************************
File Name: ALIENTEK MINISTM32 实验4 外部中断实验(库)
Revision:
Corporation:
...
//调试说明:使用JLINK V8 或ULINK2 的 SW模式,否则请取下JLINK后调试
...
int main(void)
	...
	//禁用JTAG模式,使用SW调试模式,即ULINK2或者JLINK V8带SW模式
	...
	STM_EVAL_PBInit(Button_WAKEUP, Mode_EXTI);			//设置按键外部中断方式
